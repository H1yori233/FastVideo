"""Test first-frame attention-sink anchoring to preserve the edited character.

No source injection. Just pin the first frame(s) as a persistent KV-cache sink
(sink_size>0) so every frame re-grounds the original (robot) character instead
of drifting to the model's training prior (Link).
"""
import time
...
OUT = Path("/home/hal-kaiqin/FastVideo_attninj/attn_injection_out/exp7_sink")
H, W, NF = 480, 832, 117
SINKS = [0, 1, 3]


def w_set_sink(worker, sink):
...
def main():
    ...
    for sink in SINKS:
        print(f"[set] sink={rpc(gen, w_set_sink, sink)[0]}", flush=True)
        t0 = time.time()
        gen.generate_video(prompt="", image_path=TGT, mouse_cond=mo.unsqueeze(0),
                           ...
                           output_path=str(OUT / f"sink{sink}"), save_video=True)
        print(f"    [rollout] sink{sink}: {time.time()-t0:.1f}s", flush=True)
    print("[done] exp7", flush=True)
