"""Soft feathered fg-sink (quality-first). Compare clean vs soft fg_boost."""
import time
...
OUT=Path("/home/hal-kaiqin/FastVideo_attninj/attn_injection_out/exp28_soft")
H,W,NF=480,832,201
def w_setup(worker):
    ...
    return True
def w_cfg(worker, mode, boost):
    from fastvideo.models.dits.matrixgame2.attn_injection import INJECTOR
    if mode=="off": INJECTOR.off()
    else: INJECTOR.start_fg_sink(); INJECTOR.fg_boost=boost
    return (INJECTOR.mode, boost)
...
def main():
    ...
    rpc(gen,w_cfg,"off",0); run("clean")            # sink1 only, no fg
    for b in [2.0,4.0,8.0]:
        rpc(gen,w_cfg,"soft",b); run(f"soft_b{int(b)}")
    print("[done] exp28",flush=True)
