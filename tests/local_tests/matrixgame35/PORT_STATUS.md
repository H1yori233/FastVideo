...
| DA3 metric depth | encoder/preprocessor | lazy external reuse verified | vendored `third_party/depth-anything-3`; `depth_anything_3.api.DepthAnything3` | `depth-anything/DA3NESTED-GIANT-LARGE-1.1`; Base process resolution 504, Distilled 448 | `fastvideo/memory/matrixgame35/depth_estimation.py`; lazy adapters without dependency-pin changes | adapter_complete | external_pinned_asset | local source/config pass; real CUDA standalone Shifu gate pass | I004 |
| Patch Memory / frustum reprojection | generic | port minimal inference subset | `frustum/`; `examples/wanvideo/pipeline/mosaic/` | projection-IoU/pose selection and visibility-aware z-buffer fusion | `fastvideo/memory/matrixgame35/`; Base and Distilled policies behind one package boundary | complete | not_required | direct_pinned_cpu_pass | none |
...
| memory package | `pytest fastvideo/tests/memory/test_matrixgame35_memory.py -q` | `5 passed` | focused CPU contract for Base/Distilled patch memory, dynamic context, and both DA3 adapters |
...
| lean local suite | `pytest fastvideo/tests/memory/test_matrixgame35_memory.py tests/local_tests/matrixgame35 tests/local_tests/pipelines/test_matrixgame35_pipeline_smoke.py fastvideo/tests/api/test_parser.py fastvideo/tests/api/test_compat_translation.py fastvideo/tests/api/test_extra_overrides_routing.py fastvideo/tests/api/test_schema_parity_inventory.py fastvideo/tests/api/test_presets.py -q -rs` | `161 passed, 14 skipped` | acceptance-level component parity, memory-package contract, activation smoke, and API routing; skips are CUDA/assets gates already covered by the formal Shifu jobs below |
...
| 2026-08-02 | Extract scene/context memory into `fastvideo.memory` without a generic base class | future world models need a stable home, but one implementation is insufficient evidence for a universal API | MatrixGame35 is the first model-scoped implementation; attention K/V caches remain model-owned and the package cannot import pipeline code |
