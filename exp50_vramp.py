"""exp50: frame-scheduled value anchor (ramp) vs flat. robot NF=201.
flat06 (ramp0) vs ramp04 (full by latent-f25 ~ pixel f100) vs ramp02 (full by f50).
"""
import time
...
def w_set(worker, ramp):
    from fastvideo.models.dits.matrixgame2.attn_injection import INJECTOR
    INJECTOR.fg_vhold_ramp = ramp
    return ramp
...
def main():
    ...
    for tag, ramp in [("flat06", 0.0), ("ramp04", 0.04), ("ramp02", 0.02)]:
        print("[cfg]", tag, rpc(gen, w_set, ramp)[0], flush=True)
        out = f"attn_injection_out/exp50_vramp/{tag}"
        t0 = time.time()
        gen.generate_video(
