...
OUT=Path("/home/hal-kaiqin/FastVideo_attninj/attn_injection_out/exp15_short")
H,W,NF=480,832,45
...
def main():
    ...
    for sink in [0,1,2,3]:
        print(f"[set] {rpc(gen,w_set,sink,6)[0]}",flush=True); t0=time.time()
        gen.generate_video(prompt="",image_path=TGT,mouse_cond=mo.unsqueeze(0),keyboard_cond=kb.unsqueeze(0),grid_sizes=grid,num_frames=NF,height=H,width=W,num_inference_steps=4,seed=42,output_path=str(OUT/f"sink{sink}"),save_video=True)
        print(f"  [rollout] sink{sink}: {time.time()-t0:.1f}s",flush=True)
    print("[done] exp15",flush=True)
