...
def w_setup(worker, box, boost, qsupp, sigma, vhold):
    ...
    # optional subject-region value anchor (long-rollout identity hold; ~0.6 best,
    # >0.8 hazes). 0 = validated base recipe.
    INJECTOR.fg_vhold = vhold
    return (box, boost, qsupp, vhold)
...
def main():
    ...
    ap.add_argument("--vhold", type=float, default=0.0,
                    help="subject-region value anchor (long-rollout ID; ~0.6 best)")
    ...
    print("[cfg]", rpc(gen, w_setup, args.box, args.boost, args.qsupp, args.sigma,
                       args.vhold)[0], flush=True)
