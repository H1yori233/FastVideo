...
class _Injector:
    ...
    fg_vhold_feather: bool = False  # gaussian-weight the value blend (no box seam):
    # lambda_eff(p) = vhold * w_gauss(p) -> anchor concentrates on the subject and
    # fades to 0 at the box edge, so background-in-box isn't pulled -> less haze,
    # lets vhold go higher cleanly. Same feather principle as the logit bias.
    fg_vmatch: float = 0.0  # appearance-CORRESPONDENCE value anchor (lambda). For
    # each current subject token, soft-attend to the frame-0 subject tokens by value
    # similarity and blend toward the retrieved anchor appearance -> tracks the
    # MOVING subject (vs position-matched vhold which smears under motion).
    fg_vmatch_temp: float = 1.0  # softmax temperature for the correspondence
    # frame-scheduled value anchor: ramp lambda from ~0 early (subject establishes
    # motion freely) to fg_vhold late (ground identity as drift accumulates).
    # lambda_eff = min(fg_vhold, fg_vhold * fg_vhold_ramp * start_frame); 0 = flat.
    fg_vhold_ramp: float = 0.0
    ...
    def maybe_apply(self, attn, q, k, v, sink_tokens, num_new_tokens,
        ...
        if self.mode == "fg_sink":
            ...
            vhold = self.fg_vhold
            if self.fg_vhold_ramp > 0.0:  # ramp 0 -> fg_vhold over the rollout
                vhold = min(self.fg_vhold,
                            self.fg_vhold * self.fg_vhold_ramp * start_frame)
            if self.fg_soft:
                return _fgsink_soft(q, k, v, c0, self.grid_rows, self.grid_cols,
                                    self.fg_box, boost, self.fg_sigma,
                                    qsupp, c1, vhold,
                                    self.fg_vhold_adain, self.fg_vhold_feather,
                                    self.fg_vmatch, self.fg_vmatch_temp)
...
def _fgsink_soft(q, k, v, sink_tokens, rows, cols, fg_box, fg_boost, sigma,
                 ...
                 vhold_feather=False, vmatch=0.0, vmatch_temp=1.0):
    ...
    st0 = int(sink_tokens)
    fpt = rows * cols
    # subject-region CORRESPONDENCE value anchor (tracks the moving subject) ---
    if (vmatch > 0.0 and c1 is not None and st0 >= fpt and st0 % fpt == 0):
        c1i = int(c1)
        nw = v.shape[1] - c1i
        if nw > 0 and nw % fpt == 0:
            r0, r1, cc0, cc1 = fg_box
            idx = torch.arange(fpt, device=v.device)
            keep = ((idx // cols >= r0) & (idx // cols < r1)
                    & (cc0 <= idx % cols) & (idx % cols < cc1))
            kidx = keep.nonzero().squeeze(-1)             # [S] subject token ids
            if kidx.numel() > 0:
                v = v.clone()
                v0h = v[:, :fpt][:, kidx].permute(0, 2, 1, 3).float()  # [B,H,S,D]
                sc = 1.0 / (math.sqrt(v0h.shape[-1]) * max(vmatch_temp, 1e-3))
                nfw = nw // fpt
                for j in range(nfw):
                    s = c1i + j * fpt
                    blk = v[:, s:s + fpt]
                    curh = blk[:, kidx].permute(0, 2, 1, 3).float()    # [B,H,S,D]
                    sim = torch.matmul(curh, v0h.transpose(-1, -2)) * sc
                    retr = torch.matmul(torch.softmax(sim, dim=-1), v0h)  # [B,H,S,D]
                    retr = retr.permute(0, 2, 1, 3).to(blk.dtype)      # [B,S,H,D]
                    blk = blk.clone()
                    blk[:, kidx] = (1.0 - vmatch) * blk[:, kidx] + vmatch * retr
                    v[:, s:s + fpt] = blk
    # subject-region position-matched value anchor (before attention) ---------
    if (vhold > 0.0 and c1 is not None and st0 >= fpt and st0 % fpt == 0):
        ...
        if nw > 0 and nw % fpt == 0:
            ...
            # per-token blend weight: feathered gaussian (concentrate on subject,
            # fade at box edge -> no background pull) or hard box (uniform).
            if vhold_feather:
                rc = 0.5 * (r0 + r1); cc = 0.5 * (cc0 + cc1)
                sr, sc = sigma
                fidx = idx.to(torch.float32)
                wg = torch.exp(-(((fidx // cols - rc) ** 2) / (2 * sr * sr)
                                 + ((fidx % cols - cc) ** 2) / (2 * sc * sc)))
                lam = (vhold * wg).view(1, fpt, 1, 1)     # [1,fpt,1,1] in [0,vhold]
            else:
                lam = (vhold * keep.to(torch.float32)).view(1, fpt, 1, 1)
            v = v.clone()
            ...
            for j in range(nfw):
                ...
                if vhold_adain:
                    ...
                    new = ((1.0 - lam) * cur + lam * adain).to(blk.dtype)
                else:
                    new = (1.0 - lam) * blk + lam * v0
                # lam=0 off-subject (hard) or feathers to ~0 at box edge -> blk there
                v[:, s:s + fpt] = new
    qh = q.permute(0, 2, 1, 3).float()
    kh = k.permute(0, 2, 1, 3).float()
    vh = v.permute(0, 2, 1, 3)
