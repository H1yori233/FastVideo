...
- **VALUE-ANCHOR (vhold) entirely → REJECTED (exp58):** all variants (hard / feather /
  correspondence / ramp) GHOST = frozen double-image of the subject (blend toward the
  static frame-0 subject leaves a second, motionless copy). The DINO sim-to-anchor
  metric falsely rewarded it (frozen copy ≈ anchor). Use the BASE recipe (vhold=0).
  Lesson: never trust identity-sim-to-anchor without checking motion + single-subject
  on FULL videos.
...
### exp58 (2026-06-21) — VALUE-ANCHOR REJECTED (user caught regression) — IMPORTANT
- USER review of full 297f videos: vhold0.6 output = subject "does not move at all"
  and "produces TWO characters". Verified visually (exp57 vs exp46 base): vhold gives
  a FROZEN DOUBLE-IMAGE — position-matched blend of the moving subject's values toward
  the static frame-0 subject leaves a ghost of the original pose => 2 characters + no
  motion. The DINO sim-to-anchor metric REWARDED this (a frozen anchor-copy scores high
  similarity) = textbook PROXY-METRIC FALSE POSITIVE. My exp47-53 crop/metric QA missed
  it; full-video eye review caught it. Vindicates the no-proxy-metric directive.
- FIX: revert to BASE recipe (fg_vhold=0). exp58_base/ regenerates robot/wukong/genshin/
  mc/car/dog at 297f -> SINGLE, MOVING character confirmed (top row of `_fix_check.png`).
- **CORRECTED validated best = BASE recipe: sink1/win6, feathered boost4, qsupp6,
  mid-layer-gate 0.2-0.8, sigma(5,5), NO value-anchor (vhold=0), prompt="".**
  = snapshot v1. The value-anchor (vhold/feather/vmatch) is a DEAD END (ghosting).
- Lesson: identity-sim-to-anchor is gameable by freezing/ghosting; must judge motion +
  single-subject on FULL videos, not crops/metrics. Paper draft §value-anchor RETRACTED.
