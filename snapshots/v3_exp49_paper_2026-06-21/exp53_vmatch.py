"""exp53: appearance-correspondence value anchor vs position-matched. robot 201f.
pos06 (current best, position-matched vhold0.6) vs match{04,06,08} (correspondence).
Correspondence should track the MOVING subject -> less smear, maybe higher identity.
Judge by EYE; metric corroborates.
"""
import time
...
def w_set(worker, vhold, vmatch):
    from fastvideo.models.dits.matrixgame2.attn_injection import INJECTOR
    INJECTOR.fg_vhold = vhold
    INJECTOR.fg_vhold_feather = False
    INJECTOR.fg_vmatch = vmatch
    INJECTOR.fg_vmatch_temp = 1.0
    return (vhold, vmatch)
...
def main():
    ...
    for tag, vhold, vmatch in [("pos06", 0.6, 0.0), ("match04", 0.0, 0.4),
                               ("match06", 0.0, 0.6), ("match08", 0.0, 0.8)]:
        print("[cfg]", tag, rpc(gen, w_set, vhold, vmatch)[0], flush=True)
        out = f"attn_injection_out/exp53_vmatch/{tag}"
        t0 = time.time()
        gen.generate_video(
