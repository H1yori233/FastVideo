"""exp51: action coverage — robot clean vs ours-full over actions {A, r}.
Extends generalization beyond forward(W) to strafe(A) and yaw(r). NF=201.
"""
import time
...
def w_init(worker):
    ...
    INJECTOR.fg_soft = True; INJECTOR.fg_sigma = (5.0, 5.0); INJECTOR.fg_box = BOX
    return "init"
...
def w_cfg(worker, on):
    ...
    if not on:
        ...
        INJECTOR.fg_vhold = 0.6; INJECTOR.fg_vhold_ramp = 0.0
    return f"on{on}"
...
def main():
    ...
    for act in ACTIONS:
        ...
        for tag, on in [("clean", False), ("ours", True)]:
            print("[cfg]", act, tag, rpc(gen, w_cfg, on)[0], flush=True)
            out = f"attn_injection_out/exp51_actions/{act}_{tag}"
            t0 = time.time()
            gen.generate_video(
            ...
            print(f"[done] {act}_{tag} {time.time()-t0:.1f}s", flush=True)
