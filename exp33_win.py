...
OUT=Path("/home/hal-kaiqin/FastVideo_attninj/attn_injection_out/exp33_win")
H,W,NF=480,832,201
def w_setup(worker,win):
    ...
    for m in tf.modules():
        if isinstance(m,CausalMatrixGame2SelfAttention): m.sink_size=1;m.local_attn_size=win
    tf.local_attn_size=win
    for st in worker.pipeline.stages:
        if hasattr(st,"local_attn_size"): st.local_attn_size=win
    INJECTOR.prepare(tf); INJECTOR.grid_rows=30; INJECTOR.grid_cols=52
    INJECTOR.fg_box=(14,30,21,31); INJECTOR.fg_soft=True; INJECTOR.fg_sigma=(5.0,5.0)
...
def main():
    ...
    for win in [9,12,15]:
        rpc(gen,w_setup,win); t0=time.time()
        gen.generate_video(prompt="",image_path=TGT,mouse_cond=mo.unsqueeze(0),keyboard_cond=kb.unsqueeze(0),grid_sizes=grid,num_frames=NF,height=H,width=W,num_inference_steps=4,seed=42,output_path=str(OUT/f"win{win}"),save_video=True)
        print(f"  [rollout] win{win}: {time.time()-t0:.1f}s",flush=True)
    print("[done] exp33",flush=True)
