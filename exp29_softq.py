"""Soft fg-sink + soft query suppression: identity + travel + quality (no seam)."""
import time
...
OUT=Path("/home/hal-kaiqin/FastVideo_attninj/attn_injection_out/exp29_softq")
H,W,NF=480,832,201
def w_setup(worker):
    ...
    INJECTOR.fg_box=(8,24,16,36); INJECTOR.fg_soft=True; INJECTOR.fg_sigma=(7.0,12.0)
    INJECTOR.start_fg_sink(); INJECTOR.fg_boost=2.0
    return True
def w_q(worker, qs):
    from fastvideo.models.dits.matrixgame2.attn_injection import INJECTOR
    INJECTOR.fg_qsupp=qs; return qs
...
def main():
    ...
    for qs in [3.0,6.0,10.0]:
        rpc(gen,w_q,qs); t0=time.time()
        gen.generate_video(prompt="",image_path=TGT,mouse_cond=mo.unsqueeze(0),keyboard_cond=kb.unsqueeze(0),grid_sizes=grid,num_frames=NF,height=H,width=W,num_inference_steps=4,seed=42,output_path=str(OUT/f"qs{int(qs)}"),save_video=True)
        print(f"  [rollout] qs{int(qs)}: {time.time()-t0:.1f}s",flush=True)
    print("[done] exp29",flush=True)
