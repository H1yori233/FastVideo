...
OUT=Path("/home/hal-kaiqin/FastVideo_attninj/attn_injection_out/exp17_ref")
H,W,NF=480,832,117
...
def main():
    ...
    print(f"[set] {rpc(gen,w_set,0,6)[0]}",flush=True)  # sink0 = free, vigorous, drifts
    raw=np.load(_ACT,allow_pickle=True).item()
    kb=torch.from_numpy(np.asarray(raw['keyboard'],np.float32)[:NF,:4]*3.0)  # kb3x strong drive
    mo=torch.from_numpy(np.asarray(raw['mouse'],np.float32)[:NF,:])
    grid=torch.tensor([(NF+3)//4,H//8,W//8]); t0=time.time()
    gen.generate_video(prompt="",image_path=TGT,mouse_cond=mo.unsqueeze(0),keyboard_cond=kb.unsqueeze(0),grid_sizes=grid,num_frames=NF,height=H,width=W,num_inference_steps=4,seed=42,output_path=str(OUT/"sink0_kb3x"),save_video=True)
    print(f"[rollout] sink0_kb3x: {time.time()-t0:.1f}s",flush=True); print("[done] exp17",flush=True)
