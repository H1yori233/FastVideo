...
*Working draft. Model: `mg_zelda_longlive` (MatrixGame-2, 4-step DMD, causal/streaming).
No weights are trained — inference-only.*

> ⚠️ **CORRECTION (2026-06-21, exp58).** An earlier draft made the *Subject-Region
> Value Anchor* (`fg_vhold`) the key contribution, citing a DINO identity-sim jump to
> 0.77–0.80. **That result is RETRACTED.** Full-video review showed the value anchor
> produces a **frozen double-image** (a static ghost of the frame-0 subject), and the
> DINO sim-to-anchor metric *rewarded* exactly that artifact — a proxy-metric false
> positive. **The validated method is the BASE recipe (sink + feathered boost +
> background query-suppression + structural-layer gate), with NO value anchor.** The
> value-anchor experiments now serve as a cautionary tale about embedding-similarity
> identity metrics, not as a contribution. §2(e), §4 row 6, §5b below are superseded
> by this note; the base-recipe ablation rows (1–5) and the attractor framing stand.
> Identity must be judged by *single-subject + motion on full videos*, not crops/metrics.
...
anchors the *subject region* while letting the *background travel*, (iii) decouples
the background via query-suppression so the world travels and the subject articulates,
and (iv) gates the intervention to structural layers for a coherent background. FAA
keeps an OOD humanoid recognizable, articulating, and traveling through a coherent
world over long (≈300-frame) rollouts where a plain rollout collapses to the prior
within ~50 frames — all training-free, single-subject, and moving.
*(A value-anchor variant that inflated embedding metrics was found to ghost the subject
and is retracted — see the correction note above; identity is judged on full videos.)*
