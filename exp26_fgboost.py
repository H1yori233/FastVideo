"""fg-sink + boost: amplify attention to the anchored subject (center) to beat
the Link prior, while masked periphery lets background travel. Humanoid test."""
import time
...
OUT=Path("/home/hal-kaiqin/FastVideo_attninj/attn_injection_out/exp26_fgboost")
H,W,NF=480,832,297
BOOSTS=[0.0,3.0,6.0,10.0]
def w_setup(worker):
    ...
    return True
def w_boost(worker,b):
    from fastvideo.models.dits.matrixgame2.attn_injection import INJECTOR
    INJECTOR.fg_boost=b; return b
...
def main():
    ...
    for b in BOOSTS:
        rpc(gen,w_boost,b); t0=time.time()
        gen.generate_video(prompt="",image_path=TGT,mouse_cond=mo.unsqueeze(0),keyboard_cond=kb.unsqueeze(0),grid_sizes=grid,num_frames=NF,height=H,width=W,num_inference_steps=4,seed=42,output_path=str(OUT/f"b{int(b)}"),save_video=True)
        print(f"  [rollout] b{int(b)}: {time.time()-t0:.1f}s",flush=True)
    print("[done] exp26",flush=True)
