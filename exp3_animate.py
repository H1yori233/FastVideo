"""117-frame injection experiment on animatable (game-like) targets.

Per action: record the Zelda source once, then for each target run baseline
(off) and injected (replay). Longer horizon makes camera rotation visible.
"""
import time
...
TARGETS = {
    "natzelda": f"{_COMB}/zelda_natural_scene.jpg",     # in-dist Zelda (grass ridge)
    "robotzelda": f"{_COMB}/robot_zelda_scene.jpg",     # snowy Zelda composite
    "snow": f"{_FV}/assets/images/mixkit-curve-on-a-snowy-forest-road-3317.png",  # pure OOD
}
ACTIONS = ["W.npy", "r.npy"]
LAM = 1.0
...
def main():
    ...
    for action in ACTIONS:
        ...
        for tname, tgt in TARGETS.items():
            ...
            rollout(gen, tgt, ap, gdir / f"{tname}_inj")

    print("\n[done] exp3 complete", flush=True)
