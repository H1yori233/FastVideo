...
_FV="/home/hal-kaiqin/FastVideo"; _C=f"{_FV}/assets/third-person/combine"
_ACT=f"{_FV}/examples/training/finetune/WanGame2.1_1.3b_i2v/actions_801/W.npy"
OUT=Path("/home/hal-kaiqin/FastVideo_attninj/attn_injection_out/exp39_ood")
H,W,NF=480,832,117
def w_fg(worker,box):
...
def main():
    ...
    wk=f"{_C}/wukong_zelda_scene.jpg"; car=f"{_C}/car_zelda_scene.jpg"
    rpc(gen,w_off); run(wk,"wukong_clean")
    rpc(gen,w_fg,(15,29,16,29)); run(wk,"wukong_fg")
    rpc(gen,w_off); run(car,"car_clean")
    print("[done] exp39",flush=True)
