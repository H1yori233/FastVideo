"""Diagnose forward-travel vs spinning: sink x action-scale on car_W."""
import time
...
TGT=f"{_FV}/assets/third-person/combine/car_zelda_scene.jpg"
OUT=Path("/home/hal-kaiqin/FastVideo_attninj/attn_injection_out/exp22_forward")
H,W,NF=480,832,297
CONFIGS=[(0,1.0),(0,3.0),(3,1.0),(3,3.0)]  # (sink, kbscale)
def w_set(worker,sink):
    ...
    for m in tf.modules():
        if isinstance(m,CausalMatrixGame2SelfAttention): m.sink_size=sink;m.local_attn_size=6
    return sink
...
def main():
    ...
    for sink,sc in CONFIGS:
        rpc(gen,w_set,sink); kb=torch.from_numpy(kb0*sc); t0=time.time()
        gen.generate_video(prompt="",image_path=TGT,mouse_cond=mo.unsqueeze(0),keyboard_cond=kb.unsqueeze(0),grid_sizes=grid,num_frames=NF,height=H,width=W,num_inference_steps=4,seed=42,output_path=str(OUT/f"s{sink}_kb{int(sc)}"),save_video=True)
        print(f"  [rollout] s{sink}_kb{int(sc)}: {time.time()-t0:.1f}s",flush=True)
    print("[done] exp22",flush=True)
