...
OUT=Path("/home/hal-kaiqin/FastVideo_attninj/attn_injection_out/exp30_grid")
H,W,NF=480,832,201
def w_setup(worker):
...
def main():
    ...
    for b in [3.0,4.0,5.0]:
        for qs in [4.0,8.0]:
            rpc(gen,w_cfg,b,qs); t0=time.time()
            gen.generate_video(prompt="",image_path=TGT,mouse_cond=mo.unsqueeze(0),keyboard_cond=kb.unsqueeze(0),grid_sizes=grid,num_frames=NF,height=H,width=W,num_inference_steps=4,seed=42,output_path=str(OUT/f"b{int(b)}_q{int(qs)}"),save_video=True)
            print(f"  [rollout] b{int(b)}_q{int(qs)}: {time.time()-t0:.1f}s",flush=True)
    print("[done] exp30",flush=True)
