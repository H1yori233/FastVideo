"""Crack identity-vs-travel: amplify the (viewpoint-agnostic) image cross-attn.
sink0 + kb1x (free travel) + scale image_embeds by alpha (persistent identity)."""
import time
from pathlib import Path
import cloudpickle, numpy as np, torch
from fastvideo import VideoGenerator
MODEL="/home/hal-kaiqin/models/matrix-game-2.0-zelda-longlive"
_FV="/home/hal-kaiqin/FastVideo"
_ACT=f"{_FV}/examples/training/finetune/WanGame2.1_1.3b_i2v/actions_801/W.npy"
_COMB=f"{_FV}/assets/third-person/combine"
OUT=Path("/home/hal-kaiqin/FastVideo_attninj/attn_injection_out/exp24_imgboost")
H,W,NF=480,832,297
SCENES={"robot":f"{_COMB}/robot_zelda_scene.jpg","car":f"{_COMB}/car_zelda_scene.jpg"}
ALPHAS=[1.0,4.0,8.0]
def w_set(worker):
    from fastvideo.models.dits.matrixgame2.causal_model import CausalMatrixGame2SelfAttention
    tf=worker.pipeline.get_module("transformer")
    for m in tf.modules():
        if isinstance(m,CausalMatrixGame2SelfAttention): m.sink_size=0;m.local_attn_size=6
    return True
def w_imgboost(worker, alpha):
    tf=worker.pipeline.get_module("transformer")
    if getattr(tf,'_orig_fwd',None) is None: tf._orig_fwd=tf.forward
    orig=tf._orig_fwd
    def fwd(*args,**kw):
        eh=kw.get('encoder_hidden_states_image',None)
        if eh is not None and alpha!=1.0:
            if isinstance(eh,(list,tuple)): kw['encoder_hidden_states_image']=[e*alpha for e in eh]
            else: kw['encoder_hidden_states_image']=eh*alpha
        return orig(*args,**kw)
    tf.forward=fwd
    return alpha
def rpc(gen,fn,*a): return gen.executor.collective_rpc(cloudpickle.dumps(fn),args=a)
def main():
    gen=VideoGenerator.from_pretrained(MODEL,num_gpus=1,use_fsdp_inference=False,dit_cpu_offload=False,vae_cpu_offload=False,text_encoder_cpu_offload=True,pin_cpu_memory=False)
    raw=np.load(_ACT,allow_pickle=True).item()
    kb=torch.from_numpy(np.asarray(raw['keyboard'],np.float32)[:NF,:4]); mo=torch.from_numpy(np.asarray(raw['mouse'],np.float32)[:NF,:])
    grid=torch.tensor([(NF+3)//4,H//8,W//8])
    rpc(gen,w_set)
    # warmup builds stages (alpha=1)
    for sc,img in SCENES.items():
        for a in ALPHAS:
            rpc(gen,w_imgboost,a); t0=time.time()
            gen.generate_video(prompt="",image_path=img,mouse_cond=mo.unsqueeze(0),keyboard_cond=kb.unsqueeze(0),grid_sizes=grid,num_frames=NF,height=H,width=W,num_inference_steps=4,seed=42,output_path=str(OUT/f"{sc}_a{int(a)}"),save_video=True)
            print(f"  [rollout] {sc}_a{int(a)}: {time.time()-t0:.1f}s",flush=True)
    print("[done] exp24",flush=True)
if __name__=="__main__": main()
